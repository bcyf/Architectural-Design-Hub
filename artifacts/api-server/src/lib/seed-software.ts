...
export async function seedSoftwareTools() {
  try {
    ...
    if (Number(total) >= SOFTWARE_TOOLS.length) {
      console.log(`[seed-software] ${total} software tools already present — skipping`);
      return;
    }

    console.log(`[seed-software] Found ${total} software tools — seeding ${SOFTWARE_TOOLS.length} tools…`);
    ...
    console.log(`[seed-software] Done — inserted ${SOFTWARE_TOOLS.length} software tools`);
  } catch (err) {
